...
def command() -> dict:
    ...
    parser.add_argument(
        "-f", "--folderdir", action="store", choices=["true", "false"], help="重命名父文件夹为剧集名称(可选)"
    )
    ...
    parser.add_argument(
        "-s", "--seasondir", action="store", choices=["true", "false"], help="创建季度文件夹并整理剧集(可选)"
    )
    ...
    parser.add_argument(
        ...
        "--version",
        action="version",
        version="AlistMediaRename version : v 1.5",
        help="显示版本信息",
    )
