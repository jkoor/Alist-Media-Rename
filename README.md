...
# 每批最多并发 2 个重命名任务，批次完成后等待 1 秒再继续下一批
amr [剧集关键字] -d [Alist 文件夹路径] -r 2 -t 1
...
无论是仅查询剧集信息，还是指定目录中的文件按顺序包含多个季度（例如前 12 个文件为第一季、后 12 个文件为第二季），程序都会列出 TMDB 返回的季度表。此时可输入多个表中显示的季度序号，例如 `1,2` 或 `1-3`。仅查询时会按季度顺序显示全部所选集信息；重命名时则会按相同顺序依次处理同一目录内的视频和字幕文件。

季度序号使用程序显示的“序号”列；若剧集包含特别篇（S00），请以表中的序号为准。
...
| -n, --number   |      |                 | 指定集号进行重命名           |
| -r, --limit-rate   |      |                 | 限制任务并发数；重命名时也决定每批任务数量           |
| -t, --rename-interval |      | `0` | 每批 Alist 重命名任务完成后等待的秒数，仅作用于重命名任务 |
