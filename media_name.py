...
class AlistMediaRename:
    ...
    def __init__(
        ...
        self.alist = AlistApi(self.alist_url, self.alist_user, self.alist_password, self.alist_totp)
    ...
    def tv_rename_id(
        ...
        if len(tv_info_result["seasons"]) == 1:
            ...
            if tv_season_info["request_code"] != 200:
                ...
                print(
                    f"{failure_msg} 剧集id: {tv_id}\t{tv_info_result['name']} 第 {season_number} 季\n{tv_season_info['status_message']}"
                )
                return result
        else:
            # 获取到多项匹配结果，手动选择
            ...
            # 获取剧集对应季每集信息
            tv_season_info = self.tmdb.tv_season_info(
                tv_id, season_number, language=self.tmdb_language, silent=self.debug
            )
        ...
        file_list_data = self.alist.file_list(
            path=folder_path, password=folder_password, refresh=True, silent=self.debug
        )
        ...
        video_list = list(filter(lambda x: x.split(".")[-1] in self.video_suffix_list, file_list))
        subtitle_list = list(
            filter(lambda x: x.split(".")[-1] in self.subtitle_suffix_list, file_list)
        )
        video_rename_list = list(
            map(
                lambda x, y: dict(original_name=x, target_name=y + "." + x.split(".")[-1]),
                video_list,
                episodes,
            )
        )
        ...
        for file in video_rename_list + subtitle_rename_list:
            rename_result = self.alist.rename(
                file["target_name"], folder_path + file["original_name"], silent=False
            )
            ...
            if rename_result["message"] != "success":
                ...
                if self.debug:
                    ...
                    print(
                        f"{failure_msg} 重命名失败: {file['original_name']} -> {file['target_name']}\n{rename_result['message']}"
                    )
        ...
        if self.tv_season_dir:
            ...
            season_dir_remove_result = self.alist.move(
                move_list, folder_path, season_path, silent=False
            )
    ...
    def tv_rename_keyword(
        ...
        # 查找失败则停止, 并返回结果
        if search_result["request_code"] != 200 or len(search_result["results"]) == 0:
            return result
    ...
    def movie_rename_id(self, movie_id: str, folder_path: str, folder_password=None) -> dict:
        ...
        # 根据电影id 查找TMDB电影信息
        movie_info_result = self.tmdb.movie_info(
            movie_id, language=self.tmdb_language, silent=False
        )
        ...
        video_rename_list = list(
            map(
                lambda x: dict(original_name=x, target_name=target_name + "." + x.split(".")[-1]),
                video_list,
            )
        )
        subtitle_rename_list = list(
        ...
        for files in video_rename_list, subtitle_rename_list:
            for file in files:
                rename_result = self.alist.rename(
                    file["target_name"], folder_path + file["original_name"], silent=self.debug
                )
                result["result"].append(rename_result)
                if rename_result["message"] != "success":
                    # 若部分文件命名失败， 则将success参数设为False， 并输出失败原因
        ...
        if self.media_folder_rename:
            movie_folder_rename_result = self.alist.rename(
                movie_folder_name, folder_path, silent=False
            )
    ...
    def movie_rename_keyword(self, keyword: str, folder_path: str, folder_password=None) -> dict:
        ...
        result = dict(
            success=False,
            args=dict(keyword=keyword, folder_path=folder_path, folder_password=folder_password),
            result=[],
        )
        # 使用关键词查找剧集
