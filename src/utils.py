...
class Debug:
    ...
    def output_alist_login(func):
        ...
        def wrapper(self, *args, **kwargs):
            ...
            if login_result["message"] != "success":
                print(
                    f"{Tools.ColorStr.red('[Alist●Login ✗]')} 登录失败\t{login_result['message']}"
                )
            else:
    ...
    def output_alist_file_list(func):
        ...
        def wrapper(*args, **kwargs):
            ...
            if return_data["message"] != "success":
                print(
                    f"{Tools.ColorStr.red('[Alist●List ✗]')} 获取文件列表失败: {Tools.get_argument(1, 'path', args, kwargs)}\n{return_data['message']}"
                )
                return return_data
            ...
            if return_data["data"]["content"] is None:
                print(
                    f"{Tools.ColorStr.green('[Alist●List ✓]')} 文件夹为空: {Tools.get_argument(1, 'path', args, kwargs)}"
                )
                return return_data
            ...
            # 输出格式化文件列表信息
            print(
                f"{Tools.ColorStr.green('[Alist●List●Success]')} 文件列表路径: {Tools.get_argument(1, 'path', args, kwargs)}"
            )
    ...
    def output_alist_rename(func):
        ...
        def wrapper(*args, **kwargs):
            ...
            if return_data["message"] != "success":
                print(
                    f"{Tools.ColorStr.red('[Alist●Rename ✗]')} 重命名失败: {Tools.get_argument(2, 'path', args, kwargs).split('/')[-1]} -> {Tools.get_argument(1, 'name', args, kwargs)}\n{return_data['message']}"
                )
            else:
                print(
                    f"{Tools.ColorStr.green('[Alist●Rename ✓]')} 重命名路径:{Tools.get_argument(2, 'path', args, kwargs).split('/')[-1]} -> {Tools.get_argument(1, 'name', args, kwargs)}"
                )
    ...
    def output_alist_move(func):
        ...
        def wrapper(*args, **kwargs):
            ...
            if return_data["message"] != "success":
                print(
                    f"{Tools.ColorStr.red('[Alist●Move ✗]')} 移动失败: {Tools.get_argument(2, 'src_dir', args, kwargs)} -> {Tools.get_argument(3, 'dst_dir', args, kwargs)}\n{return_data['message']}"
                )
            else:
                print(
                    f"{Tools.ColorStr.green('[Alist●Move ✓]')} 移动路径: {Tools.get_argument(2, 'src_dir', args, kwargs)} -> {Tools.get_argument(3, 'dst_dir', args, kwargs)}\n"
                )
    ...
    def output_alist_mkdir(func):
        ...
        def wrapper(*args, **kwargs):
            ...
            if return_data["message"] != "success":
                print(
                    f"{Tools.ColorStr.red('[Alist●Mkdir ✗]')} 文件夹创建失败: {Tools.get_argument(1, 'path', args, kwargs)}\n{return_data['message']}"
                )
            else:
                print(
                    f"{Tools.ColorStr.green('[Alist●Mkdir ✓]')} 文件夹创建路径: {Tools.get_argument(1, 'path', args, kwargs)}"
                )
    ...
    def output_alist_remove(func):
        ...
        def wrapper(*args, **kwargs):
            ...
            if return_data["message"] != "success":
                print(
                    f"{Tools.ColorStr.red('[Alist●Remove ✗]')} 删除失败: {Tools.get_argument(1, 'path', args, kwargs)}\n{return_data['message']}\n{return_data['message']}"
                )
            else:
                for name in Tools.get_argument(2, "name", args, kwargs):
                    print(
                        f"{Tools.ColorStr.green('[Alist●Remove ✓]')} 删除路径: {Tools.get_argument(1, 'path', args, kwargs)}/{name}"
                    )
    ...
    def output_alist_download_link(func):
        ...
        def wrapper(self, *args, **kwargs):
            ...
            if return_data["message"] == "success":
                ...
                print(
                    f"{Tools.ColorStr.red('[Alist●DL_link ✗]')} 获取文件链接失败: {Tools.get_argument(1, 'path', args, kwargs)}\n{return_data['message']}"
                )
    ...
    def output_alist_disk_list(func):
        ...
        def wrapper(self, *args, **kwargs):
            ...
            if return_data["message"] == "success":
                ...
                print(
                    f"{Tools.ColorStr.green('[Alist●Disk ✓]')} 存储列表总数: {return_data['data']['total']}"
                )
                ...
                print(
                    f"{Tools.ColorStr.red('[Alist●Disk ✗]')} 获取存储驱动失败\n{return_data['message']}"
                )
    ...
    def output_tmdb_tv_info(func):
        ...
        def wrapper(*args, **kwargs):
            ...
            if "success" in return_data and return_data["success"] is False:
                print(
                    f"{Tools.ColorStr.red('[Tmdb●tv_info ✗]')} tv_id: {Tools.get_argument(1, 'tv_id', args, kwargs)}\n{return_data['status_message']}"
                )
                return return_data
    ...
    def output_tmdb_search_tv(func):
        ...
        def wrapper(*args, **kwargs):
            ...
            if "success" in return_data and return_data["success"] is False:
                print(
                    f"{Tools.ColorStr.red('[Tmdb●search_tv ✗]')} Keyword: {Tools.get_argument(1, 'keyword', args, kwargs)}\n{return_data['status_message']}"
                )
                return return_data
            if not return_data["results"]:
                print(
                    f"{Tools.ColorStr.red('[Tmdb●search_tv ✗]')} 关键词: {Tools.get_argument(1, 'keyword', args, kwargs)}\n查找不到任何相关剧集"
                )
                return return_data

            # 格式化输出请求结果
            print(
                f"{Tools.ColorStr.green('[Tmdb●search_tv ✓]')} 关键词: {Tools.get_argument(1, 'keyword', args, kwargs)}"
            )
    ...
    def output_tmdb_tv_season_info(func):
        ...
        def wrapper(*args, **kwargs):
            ...
            if "success" in return_data and return_data["success"] is False:
                print(
                    f"{Tools.ColorStr.red('[Tmdb●tv_season_info ✗]')} 剧集id: {Tools.get_argument(1, 'tv_id', args, kwargs)}\t第 {Tools.get_argument(2, 'season_number', args, kwargs)} 季\n{return_data['status_message']}"
                )
                return return_data
            ...
            # 格式化输出请求结果
            print(
                f"{Tools.ColorStr.green('[Tmdb●tv_season_info ✓]')} {return_data['name']}"
            )
    ...
    def output_tmdb_movie_info(func):
        ...
        def wrapper(*args, **kwargs):
            ...
            if "success" in return_data and return_data["success"] is False:
                print(
                    f"{Tools.ColorStr.red('[Tmdb●movie_info ✗]')} tv_id: {Tools.get_argument(1, 'movie_id', args, kwargs)}\n{return_data['status_message']}"
                )
                return return_data

            # 格式化输出请求结果
            print(
                f"{Tools.ColorStr.green('[Tmdb●movie_info ✓]')} {return_data['title']} {return_data['release_date']}"
            )
    ...
    def output_tmdb_search_movie(func):
        ...
        def wrapper(*args, **kwargs):
            ...
            if "success" in return_data and return_data["success"] is False:
                print(
                    f"{Tools.ColorStr.red('[Tmdb●movie_info ✗]')} Keyword: {Tools.get_argument(1, 'keyword', args, kwargs)}\n{return_data['status_message']}"
                )
                return return_data

            if not return_data["results"]:
                print(
                    f"{Tools.ColorStr.red('[Tmdb●movie_info ✗]')} 关键词: {Tools.get_argument(1, 'keyword', args, kwargs)}\n查找不到任何相关电影"
                )
                return return_data

            # 格式化输出请求结果
            print(
                f"{Tools.ColorStr.green('[Tmdb●movie_info ✓]')} 关键词: {Tools.get_argument(1, 'keyword', args, kwargs)}"
            )
...
class PrintMessage:
    ...
    def print_rename_info(
        ...
        if renamed_folder_title:
            print(
                f"文件夹重命名: {folder_path.split('/')[-2]} -> {renamed_folder_title}"
            )
...
class Tools:
    ...
    def require_confirmation() -> bool:
        ...
        print("")
        while True:
            signal = input("确定要重命名吗? [回车]确定, [n]取消\t")
            if signal.lower() == "":
                return True
            if signal.lower() == "n":
                sys.exit(0)
...
class Tasks:
    ...
    def run_sync_tasks(task_list: list[Task]) -> list[TaskResult]:
        ...
        for task in task_list:
            results.append(Tools.exec_function(task.func, task.args))
        return results
        ...
        async def run_task():
            ...
            for task in task_list:
                future = loop.run_in_executor(
                    None, Tools.exec_function, task.func, task.args
                )
