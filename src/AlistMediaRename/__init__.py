...
from .utils import Tools, Tasks, Message, Debug
from .models import ApiResponseModel, Task, TaskResult, Formated_Variables
...
class Amr:
    ...
    def __init__(self, config: Union[Config, str]):
        ...
        # 登录Alist
        result_0_alist_login = self.init_login()
        self.log["result_0_alist_login"] = result_0_alist_login
        Debug.stop_on_error([result_0_alist_login])
    ...
    def init_login(self) -> TaskResult:
        ...
        if self.config.alist.guest_mode:
            result_0_alist_login = TaskResult(
                ...
                args=[],
                success=True,
                data=ApiResponseModel(success=True, status_code=200, error="", data={}),
                error="",
            )
    ...
    def tv_rename_id(
        ...
        ### ------------------------ 2. 获取剧集对应季度每集信息/文件列表 -------------------- ###
        # 选择季度
        season_number = Message.select_number(
            result_0_tmdb_tv_info.data.data["seasons"]
        )
        season_number = result_0_tmdb_tv_info.data.data["seasons"][season_number][
            "season_number"
        ]

        with self._console.status("Working..."):
            # 任务列表
            # task_2: 获取剧集对应季每集信息
            ...
            # 判断运行结果, 若出现错误则停止运行
            Debug.stop_on_error(
                [result_2_tmdb_tv_season_info, result_3_alist_file_list]
            )

        ### ------------------------ 3. 匹配剧集信息/文件列表 -------------------- ###

        fv_tv = Formated_Variables.tv(
            name=result_0_tmdb_tv_info.data.data["name"],
            original_name=result_0_tmdb_tv_info.data.data["original_name"],
            year=result_0_tmdb_tv_info.data.data["first_air_date"][:4],
            first_air_date=result_0_tmdb_tv_info.data.data["first_air_date"],
            language=result_0_tmdb_tv_info.data.data["original_language"],
            region=result_0_tmdb_tv_info.data.data["origin_country"][0],
            rating=result_0_tmdb_tv_info.data.data["vote_average"],
            season=result_2_tmdb_tv_season_info.data.data["season_number"],
        )
        ...
        episode_list_video = list(
            map(
                ...
                result_2_tmdb_tv_season_info.data.data["episodes"],
            )
        )
        ...
        with self._console.status("Working..."):
            ...
            task_5_alist_rename = Task(
                ...
                func=self.alist.rename,
                args=[
                    Tools.replace_illegal_char(folder_rename_title),
                    folder_path[:-1],
                ],
            )
            # 运行任务
        ...
        Message.print_rename_result(
            results_4_alist_rename + [result_5_alist_rename],
    ...
    def tv_rename_keyword(
        ...
        selected_number = Message.select_number(
            result_0_tmdb_search_tv.data.data["results"]
        )
        tv_id = result_0_tmdb_search_tv.data.data["results"][selected_number]["id"]
    ...
    def movie_rename_id(
        ...
        fv_movie = Formated_Variables.movie(
            name=result_0_tmdb_movie_info.data.data["title"],
            original_name=result_0_tmdb_movie_info.data.data["original_title"],
            year=result_0_tmdb_movie_info.data.data["release_date"][:4],
            release_date=result_0_tmdb_movie_info.data.data["release_date"],
            language=result_0_tmdb_movie_info.data.data["original_language"],
            region=result_0_tmdb_movie_info.data.data["origin_country"][0],
            rating=result_0_tmdb_movie_info.data.data["vote_average"],
        )
        ...
        Message.print_rename_result(
            results_3_alist_rename + [result_4_alist_rename],
            len(video_rename_list),
            len(subtitle_rename_list),
            folder_count,
        )
    ...
    def movie_rename_keyword(
        ...
        # 选择剧集
        selected_number = Message.select_number(
            result_0_tmdb_search_movie.data.data["results"]
        )
        movie_id = result_0_tmdb_search_movie.data.data["results"][selected_number][
            "id"
        ]
